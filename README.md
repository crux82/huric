...
Most of all, HuRIC is designed in order to enable processing of **Grounded Language Interpretation**  of robotic commands. In practice, the aim is to make the interpretation process dependant from the environment where the utterance is expressed. In fact, without any contentual information a command such as *"take the mug next to the keyboard"* is ambiguous: it may express the need of *picking up the mug that is near the keyboard* or to *bring the mug which position is not expressed in a position near the keyboard*. Whithout knowing the actual position of the ``mug`` and the ``keyboard``, it is not possible to assign the correct interpretation.


HuRIC is based on [Frame Semantics](https://framenet.icsi.berkeley.edu/fndrupal/) and captures cognitive information
